...
export const Home: React.FC = () => {
  ...
  // Categorize tools based on slug keywords
  ...
  // Get tools that didn't fit into main categories to avoid hiding them
  ...
  return (
    <div className="space-y-12">
      {/* FIX: The 'isHome' prop is not valid for the SEO component. Replaced with 'pageType="home"'. */}
      <SEO pageType="home" />
           ...
           <div className="bg-white rounded-2xl shadow-soft border border-brand-dark/5 overflow-hidden">
              ...
              <div className="p-2">
                 {popularTools.map((t, i) => (
                    <Link key={t.slug} to={`/${t.slug}`} className="flex items-center gap-3 group hover:bg-brand-light/40 p-3 rounded-xl transition-colors">
...
const ToolCard: React.FC<{ tool: ToolData }> = ({ tool }) => (
  <Link to={`/${tool.slug}`} className="group bg-white p-5 rounded-2xl border border-brand-dark/5 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex items-start gap-4 h-full relative overflow-hidden">
...
const CategorySection: React.FC<{ title: string; icon: string; tools: ToolData[]; color: string; accent: string }> = ({ title, icon, tools, color, accent }) => {
    ...
    <section className="scroll-mt-24">
       ...
       {tools.length > 6 && (
          <div className="mt-4 text-center md:text-right">
             <button className={`text-xs font-bold ${accent} hover:underline flex items-center justify-end gap-1 ml-auto`}>
                Tümünü İncele <span className="text-[10px]">▼</span>
             </button>
          </div>
       )}
    </section>
